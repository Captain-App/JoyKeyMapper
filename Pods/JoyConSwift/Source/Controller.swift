...
public class Controller {
    ...
    func readInitializeData(_ done: @escaping () -> Void) {
        NSLog("readInitializeData scheduled for %@", self.serialID)
        
        var isDone = false
        // Global timeout (1s settle + 5s reads)
        DispatchQueue.main.asyncAfter(deadline: .now() + 6.0) {
            if !isDone {
                NSLog("readInitializeData TIMED OUT globally for %@", self.serialID)
                isDone = true
                done()
            }
        }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
            ...
            NSLog("readInitializeData: Starting color read for %@", self.serialID)
            self.readControllerColor {
                if isDone { return }
                NSLog("readControllerColor finished for %@", self.serialID)
                self.readCalibration()
                NSLog("readCalibration called for %@", self.serialID)
    ...
    func handleInput(value: IOHIDValue) {
        ...
        /*
        if reportID == 0x21 || reportID == 0x30 {
            // Only log full mode reports to avoid spamming 0x3F (simple mode)
            NSLog("handleInput: Report ID 0x%02X for %@", reportID, self.serialID)
        }
        */
    ...
    func handleFullSensorInput(value: IOHIDValue) {
        ...
        if reportID == 0x30 {
            var hexString = ""
            for i in 0..<min(10, reportCount) {
                hexString += String(format: "%02X ", (ptr+i).pointee)
            }
            NSLog("Report 0x30 bytes: %@", hexString)
        }
        */
    ...
    public func readSPIFlash(address: UInt32, length: UInt8, handler: @escaping ([UInt8]) -> Void) {
        ...
        self.sendSubcommand(type: .getSPIFlash, data: data) { [weak self] response in
            guard let data = response else {
                NSLog("readSPIFlash failed for address 0x%04X on %@", address, self?.serialID ?? "unknown")
                // Call handler with empty data to avoid hanging
                handler([])
    ...
    func cleanUp() {
        NSLog("cleanUp: Clearing subcommands and closing device for %@", self.serialID)
        self.subcommandQueue.forEach { $0.timer?.invalidate() }
        self.subcommandQueue.removeAll()
        self.processingSubcommand?.timer?.invalidate()
        self.processingSubcommand = nil
        
        IOHIDDeviceRegisterInputValueCallback(self.device, nil, nil)
        IOHIDDeviceClose(self.device, IOOptionBits(kIOHIDOptionsTypeNone))
    }

    public var isExclusive: Bool = false

    /// Seize the device for exclusive access
    public func seize() {
        if self.isExclusive { return }
        
        NSLog("Seizing device: %@", self.serialID)
        IOHIDDeviceClose(self.device, IOOptionBits(kIOHIDOptionsTypeNone))
        let ret = IOHIDDeviceOpen(self.device, IOOptionBits(kIOHIDOptionsTypeSeizeDevice))
        if ret != kIOReturnSuccess {
            NSLog("Failed to seize device %@: %d. Fallback to shared access.", self.serialID, ret)
            // Fallback to shared access
            IOHIDDeviceOpen(self.device, IOOptionBits(kIOHIDOptionsTypeNone))
            self.isExclusive = false
        } else {
            NSLog("Successfully seized device: %@", self.serialID)
            self.isExclusive = true
        }
    }

    /// Release the device for shared access
    public func release() {
        if !self.isExclusive { return }
        
        NSLog("Releasing device: %@", self.serialID)
        IOHIDDeviceClose(self.device, IOOptionBits(kIOHIDOptionsTypeNone))
        IOHIDDeviceOpen(self.device, IOOptionBits(kIOHIDOptionsTypeNone))
        self.isExclusive = false
    }
